...
namespace MyTasks.Services
{
        ...
        ) : IAuthService
    {
        ...
        /// <summary>
        /// Consumes the current guest session and transfers its tasks to the user.
        /// The guest session is consumed first so that only one concurrent request can
        /// successfully claim it. Both operations run in the caller's transaction.
        /// </summary>
        private async Task ClaimGuestTasksIfAnyAsync(int userId)
        {
            ...
            var consumed = await _guestSessions.TryConsumeAsync(guestSessionId, now);
        ...
        public async Task<UserReadDto> RegisterAsync(RegisterDto dto)
        {
            ...
            async Task<UserReadDto> RegisterCoreAsync()
            {
                _users.AddUser(user);

                try
                {
                    // User.Id is generated by SQLite, so the user must be saved before
                    // the guest tasks can be reassigned to that user.
                    await _users.SaveChangesAsync();
                }
            ...
            return _ownerContext.IsGuest
                ? await _unitOfWork.ExecuteInTransactionAsync(RegisterCoreAsync)
        ...
        public async Task<AuthResponseDto> RefreshAsync(string rawRefreshToken)
        {
            ...
            var now = DateTime.UtcNow;
            var consumed = await _users.TryConsumeRefreshTokenAsync(existingToken.Id, now);
